...
DanmakuClient::DanmakuClient(QObject *parent) : QObject(parent), webSocket_(new QWebSocket), timer_(new QTimer(this)) {
  ...
  connect(webSocket_, &QWebSocket::binaryMessageReceived, this, [this](const QByteArray &data) {
    ...
    qDebug() << "data.size:" << data.size() << "packageLength:" << packageLength << "headerLength:" << headerLength
             << "protocolVersion:" << protocolVersion << "operation:" << operation << "seq:" << seq << "data:" << data;
...
void DanmakuClient::listen(int roomid) {
  ...
  connect(webSocket_, &QWebSocket::connected, this, [this, roomid]() {
    emit connected();

    qDebug() << "Connected:" << roomid;
